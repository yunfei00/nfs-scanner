...
class AppPathsTestCase(unittest.TestCase):
    def test_environment_overrides_all_writable_roots(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            values = {"NFS_SCANNER_CONFIG_DIR": str(root / "config"), "NFS_SCANNER_STATE_DIR": str(root / "state"), "NFS_SCANNER_LOG_DIR": str(root / "logs"), "NFS_SCANNER_DATA_DIR": str(root / "data")}
...
class DiagnosticsTestCase(unittest.TestCase):
    def test_bundle_redacts_secrets_and_excludes_measurements(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            (paths.config_dir / "settings.json").write_text(json.dumps({"token": "private", "port": "COM3"}), encoding="utf-8")
...
class RuntimeSafetyTestCase(unittest.TestCase):
    ...
    def test_emergency_request_has_distinct_worker_outcome(self) -> None:
        worker = ScanWorker(serial_port=QSerialPort(), ui_thread=QThread.currentThread(), scan_points=[], feed_rate=1000.0, dwell_seconds=0.0, motion_timeout_seconds=1.0, instrument_name="Mock-Spectrum", output_dir=Path(tempfile.gettempdir()), scan_manager=object())  # type: ignore[arg-type]
...
class ReleaseInventoryTestCase(unittest.TestCase):
    ...
    def test_windows_metadata_uses_application_version_source(self) -> None:
        ...
        self.assertIn("GetVersionNumbersString", installer)
