"""N9020A trace storage helpers.

N9020A 当前每次采集返回一条 trace。
统一按两行 CSV 存储：
1) `fre,f1,f2,...`
2) `x_y_z,v1,v2,...`
"""
...
def save_n9020a_trace_csv(
    *,
    ...
    """Save one N9020A trace using the two-line CSV layout.
...
def append_n9020a_trace_csv(
    *,
    ...
    """Append one N9020A trace row into a combined CSV file."""
...
def _validate_trace(*, frequencies: Sequence[float], values: Sequence[float]) -> tuple[list[float], list[float]]:
    ...
    if not normalized_frequencies:
        raise ValueError("N9020A 频率轴为空")
    if len(normalized_frequencies) != len(normalized_values):
        raise ValueError(
            f"N9020A 频点数量不一致: 频率={len(normalized_frequencies)}, 幅值={len(normalized_values)}"
        )
